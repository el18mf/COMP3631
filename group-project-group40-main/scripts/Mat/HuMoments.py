...
def save_character_screenshot(cv_i):
    path = f'{PROJECT_DIR}/output/cluedo_character.png'
    cv2.imwrite(path, cv_i)
...
class Detector():
    ...
    #create colour masked images
        ...
        #Cluedo Images
            ...
            # gray = cv2.cvtColor(cv_i, cv2.COLOR_BGR2GRAY)
            # faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30,30))
        
            # for (x,y,w,h) in faces:
            #     cv2.rectangle(self.confirmed_i, (x,y), (x+w), (0,255,0),2)
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
        while time.time() - start < 300:
            ...
            #This is where we will print out the screen shot of the killer
                # print("Say Cheese")
                #writing the killers details to the file
                
                # face_cascade = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
                # gray = cv2.cvtColor(self.confirmed_i, cv2.COLOR_BGR2GRAY)
                # faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(30,30))
                
                # for (x,y,w,h) in faces:
                #     cv2.rectangle(self.confirmed_i, (x,y), (x+w), (0,255,0),2)                
                
                killer = "The Killer is " + self.confirmed_killer + ".\nThey are a " + str(self.confirmed_match) + "% match to the description.\nThey were found in " + str(self.Cluedo_time) + " seconds."
                write_character_id(killer)
                save_character_screenshot(self.confirmed_i)
        else:
            print("5 minute time limit reached.")
            #check if a suspect or potential killer were found
            if self.potential_killer != "":
                #Stringing together the potential killers name, the 
                p_killer = "The searching time limit of 5 minutes was reached.\nA potential killer, ", str(self.potential_killer) + ", was located.\nWhile we cannot confirm without a doubt they are the killer, as they are only a" + str(self.potential_match) + "% match of the description.\nThey were found in " + str(self.potential_time) + " seconds."
                write_character_id(p_killer)
                save_character_screenshot(self.potential_i)
            elif self.suspect != "" and self.potential_killer == "":
                s_killer = "The searching time limit of 5 minutes was reached.\nA Suspect, ", str(self.suspect) + ", was found at the scene. \nAs they were only a " + str(self.potential_match) + "%, we cannot say for certain they are the murderer.\nThey were found at " + str(self.suspect_time) + " seconds."
                write_character_id(s_killer)
