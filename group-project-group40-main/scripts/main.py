...
class Detector():
    ...
    #Flag for matchShapes Confirmation check
        ...
        #contains the name of the current set of co-ordinates the robot is at/travelling towards
        ...
        #local variable for the storing of the name of the cluedo character
        self.confirmed_killer = ""
        #local variable for the storing of the percentage confirmed match for the cluedo character
        self.confirmed_match = 0
        #local variable for the storing of the time it took to locate the cluedo character
        self.Cluedo_time = 0
        #flag for when cluedo confirmed
        self.killer_found = False
        #Flag for when the character potrait is detected but not necessarily confirmed
        self.cluedo_detected = False
        #flag for when the robot has entered the room with the correct cluedo character
        self.correct_room_entered = False

        self.listener = tf.TransformListener()
        self.navigator = Detector.GoToPose()
        
        self.check_area_x = 0
        self.check_area_y = 0
        self.check_area_theta = 0
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
        #checks if the killer has been found yet, if it has then the robot stops working, or if the time limit is reached
        if time.time() - start < 300 or self.killer_found:           
            current = 0.8
            ...
            if self.look_for_circ != True and self.correct_room_entered != True:
                if room1_distance < room2_distance:
                    self.current_destination = 'room1'
                    self.x = input_coords[0][0]
                    self.y = input_coords[0][1]
                    dest_xy = [self.x,self.y]
                else:
                    self.current_destination = 'room2'
                    self.x = input_coords[1][0]
                    self.y = input_coords[1][1]
                    dest_xy = [self.x,self.y]
                self.theta = 90
                ...
                if success:
                    ...
                    circ_look = "I am looking for the circle now. I started doing this at: " + str(circ_look_time) + " "  + str(self.look_for_circ)
...
#==========================================================================================================================================
            ...
            if self.G_circ_found == False and self.R_circ_found == False and self.look_for_circ == True and self.correct_room_entered == False:
                check1 = "check"
                path = f'{PROJECT_DIR}/output/log_check1.txt'
                with open(path, 'w') as opened_file:
                    opened_file.write(check1)
            ...
            #Gray Scaling and Binarisation of the Red & Green Images
                ...
                for i in range(0,2):
                    if len(Circle_Contours[i]) != 0:
                        check2 = "Contours Detected of the " + str(Circle_Names[i])
                        path = f'{PROJECT_DIR}/output/log_Contour.txt'
                        with open(path, 'w') as opened_file:
                            opened_file.write(check2)
                ...
                if len(Circle_Contours[0]) != 0 or len(Circle_Contours[1]) != 0:
                    self.n += 1
                    check1 = "\ncheck: " + str(self.n)
                    path = f'{PROJECT_DIR}/output/log_check1.txt'
                    with open(path, 'w') as opened_file:
                        opened_file.write(check1)
                    if self.current_destination == 'room1':
                        self.x = input_coords[0][0]
                        self.y = input_coords[0][1]
                    elif self.current_destination == 'room2':
                        self.x = input_coords[1][0]
                        self.y = input_coords[1][1]                        
                    self.theta += 20
                    position = {'x': self.x, 'y' : self.y}
                    ...
                    rospy.sleep(1)
                    if success:
                        rotate_time = round(time.time() - start,2)
                        rotate_circ= "\nI am looking for the circle now by rotating. I started doing this at: " + str(rotate_time) + " at degrees: "  + str(self.theta)
                        path = f'{PROJECT_DIR}/output/log_rotate.txt'
                        with open(path, 'w') as opened_file:
                            opened_file.write(rotate_circ)              
                
            
        #trying to set the correct room centre co-ordinates depending on whether the red or green circle was detected                         
            elif self.correct_room_entered != True:
                if self.G_circ_found == True or self.R_circ_found == True:                                        
                    if self.G_circ_found == True:
                        ...
                        if self.current_destination == 'room1':
                            ...
                            self.x = input_coords[2][0]
                            self.y = input_coords[2][1]
                            dest_xy = [self.x,self.y]
                            
                        elif self.current_destination == 'room2':
                            # current_destination == 'room centre'
                            self.x = input_coords[3][0]
                            self.y = input_coords[3][1]   
                        ...
                        if self.current_destination == 'room1':
                            ...
                            self.x = input_coords[1][0]
                            self.y = input_coords[1][1]
                            #navigating to the alternate rooms door co-ordinates, as I found the robot would get stuck on the wall during pathfinding straight from alternate door co-ord -> centre of room co-ords
                            position = {'x': self.x, 'y' : self.y}
                            quaternion = {'r1' : 0.000, 'r2' : 0.000, 'r3' : np.sin(self.theta/2.0), 'r4' : np.cos(self.theta/2.0)}
                    ...
                    if success:
                        ...
                        correct_time = round(time.time() - start,2)
                        correct = "\nI found the correct room and entered it after spending  " + str(correct_time) + " seconds looking. Correct Room Entere = " + str(self.correct_room_entered)
                        path = f'{PROJECT_DIR}/output/log_correct_room.txt'
                        with open(path, 'w') as opened_file:
                            opened_file.write(correct)            
                    
                
            elif self.correct_room_entered == True:
                cluedo_time = round(time.time() - start,2)
                Cleudo_look = "\nI am beginning to search for the Cluedo Murderer, starting at:  " + str(cluedo_time) + " seconds." 
                path = f'{PROJECT_DIR}/output/log_cluedo_search.txt'
                with open(path, 'w') as opened_file:
                    opened_file.write(Cleudo_look) 
                ...
                #print(S_contours)
                spiral_pattern = [(0, 0), (1, 0.0), (2.0, 2.0), (0.0, 3.0), (-3.0, 3.0), (-3.0, 0.0), (-3.0, -3.0), (0.0, -3.0), (1.0, -3.0)]
                
                
                if self.cluedo_detected == False:
                    for i in range(len(spiral_pattern)):
                        next_goal_x = turtle_x + spiral_pattern[i][0]
                        next_goal_y = turtle_y + spiral_pattern[i][1]
                        for i in range(7):
                            #hopefully a 90 degree turn
                            self.theta += 45
                            position = {'x': next_goal_x, 'y' : next_goal_y}
                            quaternion = {'r1' : 0.000, 'r2' : 0.000, 'r3' : np.sin(self.theta/2.0), 'r4' : np.cos(self.theta/2.0)}

                            rospy.loginfo('Go to (%s, %s) pose', position['x'], position['y'])
                            success = self.navigator.goto(position, quaternion)
                            if success:
                                self.cluedo_detected = True
                            
                            
                    
                    
                elif self.cluedo_detected == True:    
                    cluedo_detect_time = round(time.time() - start,2)
                    Cleudo_detect = "\nThe Suspect has been detected at:  " + str(cluedo_time) + " seconds." 
                    path = f'{PROJECT_DIR}/output/log_cluedo_detected.txt'
                    with open(path, 'w') as opened_file:
                        opened_file.write(Cleudo_detect) 
                    for i in range(0,4):
                        # print(i)
                        if len(Cluedo_Contours[i]) != 0:
                            ...
                            #find the moment
                            M = cv2.moments(Cluedo_templates[i])
                            #HuMoment
                            HuMoment = cv2.HuMoments(M)
                            #Centroid
                            ...
                            if area > 100:
                                ...
                                if area > 1200:
                                    ...
                                    centroids = "\nThe centroids of the camera are: x - " + str(cv_X) + "; y - " + str(cv_X) + " and the detected image: x - " + str(cX) + "; y - " + str(cY) + ". \nTheir differences are: cv_X - cX: " str(cv_X-cX) + "; cX - cv_X: " + str(cX - cv_X)
                                    path = f'{PROJECT_DIR}/output/log_centroids.txt'
                                    with open(path, 'w') as opened_file:
                                        opened_file.write(centroids) 
                                        if area > self.check_area_theta:   
                                            if cv_X > cX:
                                                # if abs(cv_X - cX) > 10:
                                                temp_theta = - 5
                                                self.check_area_theta = area
                                            elif cv_X < cX:
                                                temp_theta = 5
                                                self.check_area_theta = area
                                            self.theta += temp_theta
                                        elif area < self.check_area_theta:
                                            self.theta += temp_theta*-1
                                        if area > self.check_area_x:
                                            self.x += 0.5
                                            self.check_area_x = area
                                        elif area < self.check_area_x:
                                            self.x -= 0.5
                                        if area > self.check_area_y:
                                            self.y += 0.5
                                            self.check_area_y = area
                                        elif area < self.check_area_y:
                                            self.y -= 0.5
                                        next_goal_x += self.x
                                        next_goal_y += self.y                                           
                                        position = {'x': next_goal_x, 'y' : next_goal_y}
...
def main(args):
    rospy.init_node("group_project", anonymous=True)
